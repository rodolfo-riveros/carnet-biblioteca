<div class="w-full bg-zinc-50 dark:bg-zinc-900 rounded-xl shadow-md overflow-hidden p-6 border border-zinc-200 dark:border-zinc-800 flex flex-col gap-4">
    ...
    <div class="overflow-x-auto w-full">
        <table class="min-w-full text-xs border border-zinc-200 dark:border-zinc-700 rounded-lg overflow-hidden">
            <thead class="bg-gray-100 dark:bg-zinc-800 sticky top-0 z-10">
                <tr>
                    ...
                    <th class="px-3 py-3 text-left font-semibold text-zinc-700 dark:text-zinc-200">PROGRAMA</th>
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                @forelse ($programasEstudio as $programaEstudio)
                    <tr class="odd:bg-white dark:odd:bg-zinc-900 even:bg-zinc-50 dark:even:bg-zinc-800/50 hover:bg-indigo-50 dark:hover:bg-indigo-900/10 transition">
                        ...
                        <td class="px-3 py-3 font-medium text-zinc-500 dark:text-zinc-400">{{ $programaEstudio->id }}</td>
                        ...
                        <td class="px-3 py-3">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="font-semibold text-zinc-900 dark:text-zinc-100 text-sm">{{ $programaEstudio->nombre }}</p>
                                    <p class="text-zinc-400 dark:text-zinc-500 text-xs">
                                        Creada {{ $programaEstudio->created_at->diffForHumans() }}
                                    </p>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-3 py-3">
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">
                                {{ $programaEstudio->institucion->nombre ?? '—' }}
                            </span>
                        </td>

                        <td class="px-3 py-3 text-center">
                            <button
                                wire:click="toggleActivo({{ $programaEstudio->id }})"
                                title="{{ $programaEstudio->activo ? 'Clic para desactivar' : 'Clic para activar' }}"
                                class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold transition
                                    {{ $programaEstudio->activo
                                        ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 hover:bg-green-200'
                                        : 'bg-zinc-100 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400 hover:bg-zinc-200' }}">
                                <span class="w-1.5 h-1.5 rounded-full {{ $programaEstudio->activo ? 'bg-green-500' : 'bg-zinc-400' }}"></span>
                                {{ $programaEstudio->activo ? 'Activo' : 'Inactivo' }}
                            </button>
                        </td>

                        <td class="px-3 py-3">
                            <div class="flex gap-1.5 justify-center items-center">
                                <a href="{{ route('programa-estudios.edit', $programaEstudio->id) }}" wire:navigate>
                                ...
                                <button wire:click="confirmDelete({{ $programaEstudio->id }})"
                                    x-data @click="$dispatch('open-modal', { name: 'modal-delete-programa-estudio' })"
        ...
        <div class="mt-4">{{ $programasEstudio->links() }}</div>
    </div>

    <div x-data="{ open: false }"
        x-on:open-modal.window="if ($event.detail.name === 'modal-delete-programa-estudio') open = true"
        x-on:close-modal.window="if ($event.detail.name === 'modal-delete-programa-estudio') open = false"
        ...
        <div class="w-full max-w-md bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-zinc-200 dark:border-zinc-700 overflow-hidden" @click.stop>
            <div class="px-6 py-5 border-b border-zinc-100 dark:border-zinc-800 flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-base font-bold text-zinc-900 dark:text-zinc-100">Eliminar programa de estudio?</h3>
            ...
            <div class="flex gap-3 px-6 py-4 bg-zinc-50 dark:bg-zinc-800/50">
                ...
                <button type="button" wire:click="deleteProgramaEstudio" wire:loading.attr="disabled"
                    class="flex-1 py-2 rounded-lg bg-red-600 hover:bg-red-700 text-white text-sm font-semibold transition">
                    <span wire:loading.remove wire:target="deleteProgramaEstudio">Sí, eliminar</span>
                    <span wire:loading wire:target="deleteProgramaEstudio">Eliminando...</span>
                </button>
            </div>
        </div>
    </div>
